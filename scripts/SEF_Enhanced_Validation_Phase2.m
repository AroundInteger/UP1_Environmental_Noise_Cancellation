function SEF_Enhanced_Validation_Phase2()
    % Phase 2: Enhanced Validation with additional statistical rigor
    % Implements multiple comparison correction, permutation testing, and Leave-One-Team-Out validation
    
    fprintf('=== SEF Enhanced Validation Phase 2 ===\n');
    ...
    results_phase2.multiple_comparison = perform_multiple_comparison_correction(data_prepared);
    
    % 2. Permutation Testing
    fprintf('2. Performing permutation testing...\n');
    results_phase2.permutation = perform_permutation_testing(data_prepared);
    
    % 3. Leave-One-Team-Out Validation
    fprintf('3. Conducting Leave-One-Team-Out validation...\n');
    results_phase2.loto_validation = perform_loto_validation(data_prepared);
    
    % 4. Computational Efficiency Assessment
    fprintf('4. Assessing computational efficiency...\n');
    results_phase2.efficiency = assess_computational_efficiency(data_prepared);
    
    % 5. Enhanced Statistical Validation
    fprintf('5. Performing enhanced statistical validation...\n');
    results_phase2.enhanced_stats = perform_enhanced_statistical_validation(data_prepared);
    
    % 6. Generate Enhanced Visualizations
    fprintf('6. Generating enhanced visualizations...\n');
    generate_enhanced_visualizations(results_phase2);
    
    % 7. Save Results
    save('outputs/results/sef_enhanced_validation_phase2.mat', 'results_phase2');
    
    % 8. Generate Enhanced Report
    generate_enhanced_validation_report(results_phase2);
    
    fprintf('✓ Phase 2 Enhanced Validation completed successfully!\n');
end

function mc_results = perform_multiple_comparison_correction(data)
    ...
    for kpi = 1:n_kpis
        for season = 1:n_seasons
            ...
            if sum(season_mask) > 10 % Minimum sample size
                ...
                % Calculate SEF
                sef = calculate_sef_enhanced(season_a, season_b);
                sef_values(end+1) = sef;
                
                % Bootstrap test for SEF > 1
                [p_val, ~] = bootstrap_sef_test(season_a, season_b, 1000);
    ...
    mc_results.fdr_corrected = fdr_correction(p_values);
    mc_results.holm_corrected = holm_correction(p_values);
...
function perm_results = perform_permutation_testing(data)
    ...
    observed_sef = calculate_sef_enhanced(team_a_perf(:), team_b_perf(:));
    ...
    for i = 1:n_permutations
        ...
        perm_sef_values(i) = calculate_sef_enhanced(perm_a(:), perm_b(:));
    end
...
function loto_results = perform_loto_validation(data)
    ...
    % This is a simplified approach for the validation
    team_a_ids = team_ids;
    team_b_ids = team_ids;
    ...
    full_sef = calculate_sef_enhanced(team_a_perf(:), team_b_perf(:));
    ...
    for i = 1:n_teams
        ...
        if sum(exclude_mask) > 50 % Minimum sample size
            ...
            sef_without = calculate_sef_enhanced(team_a_subset(:), team_b_subset(:));
...
function eff_results = assess_computational_efficiency(data)
    ...
    for i = 1:length(sample_sizes)
        ...
        for j = 1:n_tests
            ...
            tic;
            sef = calculate_sef_enhanced(test_a(:), test_b(:));
            times(j) = toc;
...
function enhanced_stats = perform_enhanced_statistical_validation(data)
    % Enhanced statistical validation with additional tests
    ...
    % 1. Normality tests
    enhanced_stats.normality = perform_normality_tests(relative_perf);
    
    % 2. Stationarity tests
    enhanced_stats.stationarity = perform_stationarity_tests(relative_perf);
    
    % 3. Autocorrelation tests
    enhanced_stats.autocorrelation = perform_autocorrelation_tests(relative_perf);
    
    % 4. Heteroscedasticity tests
    enhanced_stats.heteroscedasticity = perform_heteroscedasticity_tests(team_a_perf(:), team_b_perf(:));
    
    % 5. Cross-validation
    enhanced_stats.cross_validation = perform_cross_validation(team_a_perf, team_b_perf);
    
    fprintf('    Normality tests completed\n');
    fprintf('    Stationarity tests completed\n');
    fprintf('    Autocorrelation tests completed\n');
    fprintf('    Heteroscedasticity tests completed\n');
    fprintf('    Cross-validation completed\n');
end

function sef = calculate_sef_enhanced(team_a, team_b)
    % Enhanced SEF calculation with error handling
...
function [p_value, sef_dist] = bootstrap_sef_test(team_a, team_b, n_bootstrap)
    % Bootstrap test for SEF > 1
    ...
    % Calculate observed SEF
    observed_sef = calculate_sef_enhanced(team_a, team_b);
    ...
    for i = 1:n_bootstrap
        ...
        sef_dist(i) = calculate_sef_enhanced(boot_a, boot_b);
    end
...
function corrected_p = holm_correction(p_values)
...
function normality_results = perform_normality_tests(data)
    ...
    if length(data) >= 3 && length(data) <= 5000
        [~, normality_results.shapiro_wilk_p] = swtest(data);
    else
        normality_results.shapiro_wilk_p = NaN;
    end
    ...
    % D'Agostino test
    normality_results.dago_p = dagostino_k2test(data);
    
    % Overall assessment
    p_values = [normality_results.shapiro_wilk_p, normality_results.ks_p, ...
                normality_results.jb_p, normality_results.dago_p];
    valid_p = p_values(~isnan(p_values));
    normality_results.overall_normal = all(valid_p > 0.05);
    normality_results.n_tests = length(valid_p);
end

function stationarity_results = perform_stationarity_tests(data)
    % Perform stationarity tests
    
    stationarity_results = struct();
    
    % Augmented Dickey-Fuller test
    [~, stationarity_results.adf_p] = adftest(data);
    
    % KPSS test
    [~, stationarity_results.kpss_p] = kpsstest(data);
    
    % Overall assessment
    stationarity_results.stationary = stationarity_results.adf_p < 0.05 && stationarity_results.kpss_p > 0.05;
end

function autocorr_results = perform_autocorrelation_tests(data)
    % Perform autocorrelation tests
    
    autocorr_results = struct();
    
    % Ljung-Box test
    [~, autocorr_results.ljung_box_p] = lbqtest(data);
    
    % Durbin-Watson test
    autocorr_results.durbin_watson = dwtest(data);
    
    % Overall assessment
    autocorr_results.has_autocorr = autocorr_results.ljung_box_p < 0.05;
end

function hetero_results = perform_heteroscedasticity_tests(team_a, team_b)
    % Perform heteroscedasticity tests
    
    hetero_results = struct();
    
    % Breusch-Pagan test
    [~, hetero_results.bp_p] = bptest(team_a, team_b);
    
    % White test
    [~, hetero_results.white_p] = whitetest(team_a, team_b);
    
    % Overall assessment
    hetero_results.has_hetero = hetero_results.bp_p < 0.05 || hetero_results.white_p < 0.05;
end

function cv_results = perform_cross_validation(team_a_perf, team_b_perf)
    ...
    for i = 1:k
        ...
        sef_folds(i) = calculate_sef_enhanced(fold_a(:), fold_b(:));
    end
...
function generate_enhanced_visualizations(results)
    % Generate enhanced visualizations for Phase 2
    ...
    sgtitle('Phase 2: Enhanced Validation Results');
    saveas(gcf, 'outputs/figures/enhanced_validation/phase2_enhanced_validation.png');
    saveas(gcf, 'outputs/figures/enhanced_validation/phase2_enhanced_validation.fig');
    close;
    
    fprintf('    Enhanced visualizations saved to outputs/figures/enhanced_validation/\n');
end

function generate_enhanced_validation_report(results)
    ...
    % Generate report
    report_file = 'outputs/results/sef_enhanced_validation_phase2_report.txt';
    fid = fopen(report_file, 'w');
    
    fprintf(fid, 'SEF Enhanced Validation Phase 2 Report\n');
    fprintf(fid, '=====================================\n\n');
    ...
    fprintf(fid, 'Stationary: %s\n', string(results.enhanced_stats.stationarity.stationary));
    fprintf(fid, 'Has autocorrelation: %s\n', string(results.enhanced_stats.autocorrelation.has_autocorr));
    fprintf(fid, 'Has heteroscedasticity: %s\n', string(results.enhanced_stats.heteroscedasticity.has_hetero));
    ...
    fprintf(fid, '----------\n');
    fprintf(fid, 'Phase 2 enhanced validation completed successfully.\n');
