%% ========================================================================
% RUN USER ANALYSIS - SIMPLIFIED INTERFACE
% ========================================================================
% 
% This is a simplified interface for users to run the Interactive Data
% Analysis Pipeline with their own data.
%
% Usage:
...
% Author: AI Assistant
% Date: 2024
% Purpose: User-friendly analysis interface
%
% ========================================================================

clear; clc; close all;

fprintf('=== USER DATA ANALYSIS INTERFACE ===\n');
fprintf('Welcome to the Interactive Data Analysis Pipeline!\n\n');

fprintf('This tool will analyze your data to determine:\n');
fprintf('1. Whether environmental noise exists (η)\n');
fprintf('2. Optimal measurement strategy (relative vs absolute)\n');
fprintf('3. Expected SNR improvements\n');
fprintf('4. Clear recommendations for your data\n\n');

%% Step 1: Data File Selection
fprintf('STEP 1: Select your data file\n');
fprintf('=============================\n');

% Check for files in data/raw/
data_dir = 'data/raw/';
if exist(data_dir, 'dir')
    files = dir(fullfile(data_dir, '*.csv'));
    if ~isempty(files)
        fprintf('Available CSV files in %s:\n', data_dir);
        for i = 1:length(files)
            fprintf('  %d. %s\n', i, files(i).name);
        end
        
        choice = input('\nSelect file number (or 0 to specify custom path): ');
        
        if choice > 0 && choice <= length(files)
            file_path = fullfile(data_dir, files(choice).name);
            fprintf('Selected: %s\n', files(choice).name);
        else
            file_path = input('Enter full path to your CSV file: ', 's');
        end
    else
...
try
    ...
    config.verbose = true;
    config.interactive = false; % Automated mode
    config.user_file = file_path;
    
    % Run the main analysis pipeline
    runInteractiveAnalysis(data, config);
    
    fprintf('\n✓ Analysis completed successfully!\n');
    
catch ME
    fprintf('\n✗ Error during analysis: %s\n', ME.message);
    fprintf('Please check your data file and try again.\n');
    return;
end
...
fprintf('4. Implement the recommended measurement strategies\n');
fprintf('5. Monitor performance improvements\n\n');

fprintf('For questions or support, please refer to the documentation.\n');
fprintf('Thank you for using the Interactive Data Analysis Pipeline!\n');

%% Helper Function
function runInteractiveAnalysis(data, config)
    % Run the interactive analysis pipeline with user data
    ...
    report.analysis_type = 'User Data Analysis';
...
function createAnalysisVisualization(strategy_results, env_analysis, decision_results, output_dir)
    ...
    sgtitle('User Data Analysis Results');
...
function generateTextReport(report, filename)
    ...
    fprintf(fid, 'USER DATA ANALYSIS REPORT\n');
    fprintf(fid, '========================\n\n');
